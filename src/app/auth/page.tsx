...
export default function AuthPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error === 'USER_ALREADY_EXISTS' ? 'Пользователь уже зарегистрирован.' : data.error === 'SUPABASE_NOT_CONFIGURED' ? 'Авторизация временно недоступна. Проверьте настройки Supabase.' : 'Не удалось выполнить вход. Проверьте данные.');
        setBusy(false);
        return;
      }
