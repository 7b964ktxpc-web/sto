...
const labels:Record<string,string>={active:'Активна',pending:'На модерации',suspended:'Приостановлена',rejected:'Отклонена'};
...
export default function AdminBusinessesPage(){
 ...
 return <main className="page"><header className="hero"><div className="hero-inner"><span className="eyebrow">STO NSK · Admin</span><h1>СТО</h1><p>Поиск и оперативное управление организациями marketplace.</p></div></header><div className="content">
   <section className="card"><form onSubmit={submit} style={{display:'grid',gridTemplateColumns:'minmax(0,1fr) 220px auto',gap:10}}><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Название или телефон"/><select value={status} onChange={e=>setStatus(e.target.value)}>{statuses.map(s=><option key={s} value={s}>{s?labels[s]:'Все статусы'}</option>)}</select><button className="primary" type="submit">Искать</button></form></section>
   ...
   {loading?<div className="empty">Загружаем список…</div>:items.length===0?<div className="empty">По заданным условиям ничего не найдено.</div>:items.map(b=><div className="service-row" key={b.id}><div style={{flex:1,minWidth:0}}><strong>{b.name}</strong><div className="muted">{b.address||'Адрес не указан'}{b.phone?` · ${b.phone}`:''}</div><div className="muted">★ {b.rating||0} · {b.review_count||0} отзывов</div></div><span className="pill">{labels[b.status]||b.status}</span>{b.slug&&<a className="pill" href={`/novosibirsk/sto/${b.slug}`}>Открыть</a>}<a className="primary" href={`/admin/businesses/${b.id}`}>Управление</a></div>)}
   </section>
