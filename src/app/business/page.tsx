...
export default function BusinessDashboard(){
  ...
  if(!data) return <main className="content"><div className="card">Загружаем dashboard…</div></main>;
  const activeQueue=(data.queue?.queue_entries??[]).filter((x:any)=>['WAITING','CALLED','IN_SERVICE'].includes(x.status));
  return <main className="page"><header className="hero"><div className="hero-inner"><span className="eyebrow">STO NSK · Кабинет СТО</span><h1>{data.business.name}</h1><p>Операционный центр: записи, посты, очередь и услуги.</p></div></header><div className="content">
    <div className="feature-grid"><div className="feature"><strong>Сегодня</strong><span style={{fontSize:28,fontWeight:800}}>{data.appointments.length}</span><span className="muted">записей</span></div><div className="feature"><strong>В работе</strong><span style={{fontSize:28,fontWeight:800}}>{data.appointments.filter(x=>x.status==='IN_SERVICE').length}</span><span className="muted">автомобилей</span></div><div className="feature"><strong>Рейтинг</strong><span style={{fontSize:28,fontWeight:800}}>★ {data.business.rating||0}</span></div></div>
    <div className="layout" style={{marginTop:16}}><section className="results">
      <div className="card"><div className="toolbar"><div><h2>Расписание сегодня</h2><div className="muted">Day view</div></div><span className="pill">Календарь</span></div>{data.appointments.length===0?<div className="empty muted">Сегодня записей нет.</div>:data.appointments.map((a:any)=><div className="service-row" key={a.id}><strong>{new Date(a.starts_at).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}</strong><span>{a.status}</span><span className="pill">{a.workstation_id?'Пост назначен':'Пост не назначен'}</span></div>)}</div>
