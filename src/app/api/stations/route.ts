...
export async function GET(request: Request) {
  ...
  const locationByBusiness = new Map<string, any>();
  for (const location of locations ?? []) locationByBusiness.set(location.business_id, location);

  const { data: businessServices, error: serviceError } = await db
    .from('business_services')
    .select('id,business_id,service_id,price,duration_minutes,services(id,name)')
    .in('business_id', ids)
    .eq('is_active', true);
  ...
  const stations = (businesses ?? [])
    ...
    .filter((station: any) => {
      const matchesQuery = !query
        ...
        || station.station_services.some((x: any) => x.services?.name?.toLowerCase().includes(query));
      const matchesService = !service
        || station.station_services.some((x: any) => x.service_id === service || x.services?.name?.toLowerCase().includes(service.toLowerCase()));
