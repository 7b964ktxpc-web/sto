...
export async function PUT(request: Request) {
  ...
  if (hours.some((h: any) => !DAYS.has(Number(h.weekday)) || !validTime(h.starts_at) || !validTime(h.ends_at) || (h.break_starts_at && !validTime(h.break_starts_at)) || (h.break_ends_at && !validTime(h.break_ends_at)))) {
    return NextResponse.json({ error: 'INVALID_WORKING_HOURS' }, { status: 400 });
  }
  if (hours.some((h: any) => h.ends_at <= h.starts_at || ((h.break_starts_at || h.break_ends_at) && (!h.break_starts_at || !h.break_ends_at || h.break_starts_at < h.starts_at || h.break_ends_at > h.ends_at || h.break_starts_at >= h.break_ends_at)))) {
    return NextResponse.json({ error: 'INVALID_WORKING_HOURS_RANGE' }, { status: 400 });
  }
  if (new Set(hours.map((h:any) => Number(h.weekday))).size !== hours.length) {
    return NextResponse.json({ error: 'DUPLICATE_WEEKDAY' }, { status: 400 });
  }
  if (exceptions.some((e:any) => !/^\d{4}-\d{2}-\d{2}$/.test(String(e.exception_date)) || (!e.is_closed && (!validTime(e.starts_at) || !validTime(e.ends_at) || e.ends_at <= e.starts_at)))) {
    return NextResponse.json({ error: 'INVALID_EXCEPTION' }, { status: 400 });
  }
  ...
  if (normalizedHours.length) {
    const { error } = await db.from('working_hours').insert(normalizedHours);
    if (error) return NextResponse.json({ error: 'SCHEDULE_SAVE_FAILED' }, { status: 503 });
  }
  ...
  if (normalizedExceptions.length) {
    const { error } = await db.from('working_exceptions').insert(normalizedExceptions);
    if (error) return NextResponse.json({ error: 'EXCEPTIONS_SAVE_FAILED' }, { status: 503 });
  }

  await db.from('audit_logs').insert({ actor_user_id: auth.user.id, business_id: businessId, action: 'UPDATE', entity_type: 'BUSINESS_SCHEDULE', entity_id: businessId, reason: 'Admin schedule update', before_data: { hours: beforeHours ?? [], exceptions: beforeExceptions ?? [] }, after_data: { hours: normalizedHours, exceptions: normalizedExceptions }, ip: requestIp(request), user_agent: request.headers.get('user-agent') });
