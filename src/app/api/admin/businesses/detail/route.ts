...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data: business, error: businessError } = await db
      .from('businesses')
      .select('id,name,slug,status,rating,review_count,phone,description,created_at')
      .eq('id', id)
      .maybeSingle();
    ...
    const owner = Array.isArray(ownerMember.data?.user) ? ownerMember.data.user[0] : ownerMember.data?.user;
