...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data: business, error: businessError } = await db.from('businesses').select('id,name,slug,status,rating,review_count,phone,description,created_at').eq('id', id).maybeSingle();
    if (businessError) throw businessError;
    if (!business) return NextResponse.json({ error: 'BUSINESS_NOT_FOUND' }, { status: 404 });
    const [location, ownerMember, services, employees, workstations, appointments] = await Promise.all([
      db.from('business_locations').select('address').eq('business_id', id).eq('is_primary', true).maybeSingle(),
      db.from('business_members').select('user:users(display_name,phone,email),role').eq('business_id', id).in('role', ['BUSINESS_OWNER','BUSINESS_MANAGER']).order('created_at', { ascending: true }).limit(1).maybeSingle(),
    ...
    return NextResponse.json({ business: { ...business, address: location.data?.address ?? null, email: ownerMember.data?.user?.email ?? null, owner_name: ownerMember.data?.user?.display_name ?? null }, services: services.data ?? [], employees: employees.data ?? [], workstations: workstations.data ?? [], appointments: appointments.data ?? [] });
  } catch (error) {
