...
export async function PATCH(request:Request){
 const auth=await requirePlatformRole(roles); if(!auth)return NextResponse.json({error:'ADMIN_ACCESS_REQUIRED'},{status:403});
 try{
  ...
  if(readError||!before)return NextResponse.json({error:'SERVICE_NOT_FOUND'},{status:404});
  ...
  await db.from('audit_logs').insert({actor_id:auth.user.id,action:'business_service.updated',entity_type:'business_service',entity_id:id,metadata:{before,after}});
  return NextResponse.json({service:after});
 }catch(error){console.error(error);return NextResponse.json({error:'SERVICE_UPDATE_FAILED'},{status:500});}
}
