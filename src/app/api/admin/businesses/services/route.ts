...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const { data: before, error: readError } = await db
      ...
      .select('id,price,duration_minutes,is_active,business_id')
      .eq('id', id)
      .maybeSingle();
    ...
    if (!Number.isFinite(next.price) || next.price < 0 || !Number.isFinite(next.duration_minutes) || next.duration_minutes <= 0) {
    ...
    await db.from('audit_logs').insert({
      ...
      action: 'business_service.updated',
