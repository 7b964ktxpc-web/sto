...
export async function POST(request: Request) {
  try {
    ...
    await db.from('sessions').insert({ user_id: created.user.id, token_hash: tokenHash, expires_at: new Date(Date.now() + 1000 * 60 * 60 * 24 * 30).toISOString(), user_agent: request.headers.get('user-agent'), ip: request.headers.get('x-forwarded-for')?.split(',')[0]?.trim() ?? null });
