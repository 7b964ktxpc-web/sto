create or replace function public.create_appointment_atomic_session(
  p_user_id uuid,
  p_business_id uuid,
  p_car_id uuid,
...
begin
  if not exists (select 1 from users where id = p_user_id and not is_blocked) then raise exception 'USER_NOT_FOUND'; end if;
  select bs.duration_minutes, bs.required_employee, bs.required_workstation into v_duration, v_required_employee, v_required_workstation
  from business_services bs join businesses b on b.id = bs.business_id
  ...
  insert into appointments(business_id,user_id,car_id,business_service_id,starts_at,ends_at,status,notes)
  values (p_business_id,p_user_id,p_car_id,p_business_service_id,p_starts_at,v_ends_at,'CONFIRMED',p_notes)
  returning id into v_appointment_id;
  if p_employee_id is not null then
    insert into appointment_resources(appointment_id,business_id,resource_type,resource_id,reserved_range)
    values (v_appointment_id,p_business_id,'EMPLOYEE',p_employee_id,tstzrange(p_starts_at,v_ends_at,'[)'));
  end if;
  if p_workstation_id is not null then
    insert into appointment_resources(appointment_id,business_id,resource_type,resource_id,reserved_range)
    values (v_appointment_id,p_business_id,'WORKSTATION',p_workstation_id,tstzrange(p_starts_at,v_ends_at,'[)'));
  end if;
  insert into appointment_events(appointment_id,actor_user_id,event_type,payload)
  values (v_appointment_id,p_user_id,'BOOKING_CREATED',jsonb_build_object('source','marketplace-web'));
  return v_appointment_id;
exception when exclusion_violation then
  raise exception 'SLOT_ALREADY_TAKEN';
end;
$$;
revoke execute on function public.create_appointment_atomic_session(uuid,uuid,uuid,uuid,timestamptz,uuid,uuid,text) from public, anon, authenticated;
grant execute on function public.create_appointment_atomic_session(uuid,uuid,uuid,uuid,timestamptz,uuid,uuid,text) to service_role;
